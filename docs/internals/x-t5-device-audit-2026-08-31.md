...
- Raw-conversion profile constants: the declared `profile_code: "ff179502"` (8 chars) in `fml/camera.cue` is arithmetically inconsistent with the declared `header_padding: 0x1ee`, which matches a 7-character code - exactly the length of the live `D184` first field `F179502`. `libfuji`'s X-T5 captures embed `FF129504` with values at fixed offset `0x201`. Resolving this needs a `GetDevicePropValue(0xD185)` capture with a RAF loaded for conversion; see the reconciliation section of [fuji-ptp-ecosystem-research](fuji-ptp-ecosystem-research.md).
- The 601-byte (23-field) vs 629-byte (29-field) D185 read/write asymmetry in `libfuji`'s captures is unexplained; the FML declares 29 for both directions.

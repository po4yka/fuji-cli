...
- Print `0xD18C` in `discover simulation` output, so the read-only namespace experiment in the design section above can be run through the existing CLI (addressed: `discover simulation` now reads `0xD18C` first, and `discover property CODE` reads any single property; neither has been run on a camera yet).
- Implement the `NamespaceSignal` read of `0xD18D` in `crates/fujicli-dev/src/decision.rs` and run the read-only namespace experiment above as the maintainer's next step, ahead of any mutating `simulation-namespace` probe run (code addressed: the probe reads `0xD18D` after the write and decides against operator-declared slot names, verified against fakes only; the read-only experiment and any device run remain pending).

...
pub(crate) fn generate(
    ...
    let (ptp_serde_impl, simulation_setting_impl) = if let Some(code) = prop_code {
        let serde = generate_ptp_serde_impl(&type_name, &repr_type)
            .with_context(|| format!("generating binrw codec impls for integer option `{id}`"))?;
...
mod tests {
    ...
    #[test]
    fn generated_integer_lookup_uses_binrw_manual_codec() {
        let type_name = Ident::new("DynamicRange", Span::call_site());
        let repr_type = Ident::new("u16", Span::call_site());

        let generated = generate_ptp_serde_impl(&type_name, &repr_type)
            .expect("integer lookup wire codec generation must succeed")
            .to_string();
        ...
        assert!(
            ...
            "generated integer lookup must use the binrw manual codec: {generated}",
        );
    }

    #[test]
    fn generated_integer_lookup_conversion_profile_uses_binrw_io() {
        let type_name = Ident::new("DynamicRange", Span::call_site());
        let repr_type = Ident::new("u16", Span::call_site());
        ...
        let generated = generate_conversion_profile_impl(&type_name, &repr_type, &repr_type_32)
            .expect("integer lookup conversion profile generation must succeed")
            .to_string();
        ...
        assert!(
            (uses_binrw_traits || uses_binrw_context)
                && generated.contains("try_from_wire")
                && !generated.contains("ptp_cursor"),
            "generated integer lookup conversion profile must use binrw I/O: {generated}",
        );
    }
}
